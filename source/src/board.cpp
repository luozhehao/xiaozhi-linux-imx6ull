...
Board::Board() {
    ...
    // 初始化音乐播放器
    music_svr_ = nullptr;
    music_svr_ = new MusicService();
    if (music_svr_) {
        std::cout << "Board, music_svr_ init success" << std::endl;
    }else{
        std::cout << "Board, music_svr_ init fail" << std::endl;
    }
...
MusicService* Board::GetMusic() {
    std::cout << "### GetMusic ... 001 " << std::endl;
    return music_svr_;
}