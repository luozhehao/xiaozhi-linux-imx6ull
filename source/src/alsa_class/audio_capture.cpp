...
bool AudioCapture::init() {
    ...
    std::cout << "Audio capture device initialized: " << params_.device_name << std::endl;
    ...
    input_channels_ = actual_params_.channels;
    input_sample_rate_ = actual_params_.sample_rate;
...
bool AudioCapture::InputData(std::vector<int16_t>& data) {
    // int samples = Read(data.data(), data.size());
    int samples = snd_pcm_readi(pcm_handle_, data.data(), data.size());
    if (samples == -EPIPE) {
        // 处理overrun
        snd_pcm_prepare(pcm_handle_);
        // continue;
    } else if (samples < 0) {
        std::cerr << "ALSA read error: " << snd_strerror(samples) << std::endl;
        // break;
    }
