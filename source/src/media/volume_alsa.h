...
private:
    ...
    snd_mixer_elem_t* element_headphone_;
    std::string device_;
    std::string control_;
    std::string control_headphone_;
    
public:
    VolumeController(const std::string& device = "default", 
                    const std::string& control = "Speaker",
                    const std::string& control_hp = "Headphone")
        : mixer_(nullptr), element_(nullptr), element_headphone_(nullptr), device_(device), control_(control), control_headphone_(control_hp) {
        open_mixer();
    }
    ...
    void set_volume(int volume) {
        ...
        snd_mixer_selem_set_playback_volume_all(element_headphone_, volume);          //element_headphone_
        snd_mixer_selem_set_playback_switch_all(element_, 1); // 确保开启
        snd_mixer_selem_set_playback_switch_all(element_headphone_, 1); // 确保开启
    }
...
private:
    void open_mixer() {
        ...
        // 查找控制元素
        element_headphone_ = find_control_element(control_headphone_);
        if (!element_headphone_) {
            snd_mixer_close(mixer_);
            throw std::runtime_error("Cannot find control: " + control_headphone_);
        }
