...
void AudioPlayer::init_alsa() {
    ...
    // 打开PCM设备                //  SND_PCM_NONBLOCK
    if ((err = snd_pcm_open(&alsa_handle, "default", SND_PCM_STREAM_PLAYBACK, SND_PCM_NONBLOCK)) < 0) {
...
/********** luo ***********/
/************ 100adk********************/
        // std::cout << "decBufToPlayBuf start ... " << "\n";
        // decBufToPlayBuf(pcm_data, playBuffer);
        // std::cout << "decBufToPlayBuf finish ..." << "\n";

        // int err = snd_pcm_writei(alsa_handle, playBuffer, 640);
        // std::cout << "[PLAY] writei took " << "  wrote " << err << " frames\n";
        // if (err == -EPIPE) {
        // // 处理underrun
        // std::cerr << "ALSA underrun occurred " << std::endl;
        // snd_pcm_prepare(alsa_handle);
...
/***********100adk***********************/
        ...
        // 处理每一帧
        // int play_size = frame.size() * 2;
        // std::vector<int16_t> play_data(play_size);
        // for (int i =0; i< frame.size(); i++)
        // {
        //     play_data[2 * i] = frame[i];
        //     play_data[2 * i + 1] = frame[i];
        // }

        // // 在 AlsaPlayer::run() 内
        // auto t1 = std::chrono::steady_clock::now();
        // snd_pcm_sframes_t written = snd_pcm_writei(handle_, data, samples_left / CHANNELS);
        // auto t2 = std::chrono::steady_clock::now();
        // auto dt = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
        // std::cout << "[PLAY] writei took " << dt << " ms, wrote " << written << " frames\n";
