...
void McpServer::AddCommonTools() {
    ...
    std::cout << "### AddCommonTools ... 003 " << std::endl;
    ...
    std::cout << "### AddCommonTools ... 004 " << std::endl;


    auto music = board.GetMusic();
    // music->Play("晴天", "");
    std::cout << "### AddCommonTools ... 005 " << std::endl;
    if (music) {
         AddTool("self.music.play_song",
             ...
             [music](const PropertyList& properties) -> ReturnValue {
                ...
                //  auto artist_name = properties["artist_name"].value<std::string>();
                 ...
                 if (!music->Play(song_name, artist_name)) {
                    std::string success_str = "{\"success\": false, \"message\": \"获取音乐资源失败\"}";
                     return success_str;
                 }
                //  auto download_result = music->GetDownloadResult();
                //  ESP_LOGI(TAG, "Music details result: %s", download_result.c_str());
                 std::string fail_str = "{\"success\": true, \"message\": \"音乐开始播放\"}";
                 return fail_str;
             });
    }    
    std::cout << "### AddCommonTools ... 006 " << std::endl;
...
void McpServer::SendMcpMessage(const std::string& payload){
    ...
    std::string message = "{\"session_id\":\"" + mcp_session_id + "\",\"type\":\"mcp\",\"payload\":" + payload + "}";
    // SendText(message);
    WebsocketProtocol::GetInstance().websocket_send_text(message.data(), message.size());
}
...
void McpServer::GetToolsList(int id, const std::string& cursor){
    ...
    std::cout << "tools_.size() = " <<  tools_.size() << std::endl;
    ...
    while (it != tools_.end()) {
        ...
        std::cout << "tool_json = " + tool_json << std::endl;
