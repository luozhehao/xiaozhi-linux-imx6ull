...
// 添加类型别名
// using ReturnValueV = std::variant<bool, int, std::string>;

// 定义 variant 类型
// typedef boost::variant<int, std::string, double> ReturnValue;
using ReturnValue = boost::variant<bool, int, std::string>;
...
private:
    ...
    boost::variant<bool, int, std::string> value_;
    bool has_default_value_;
    // std::optional<int> min_value_;  // 新增：整数最小值
    // std::optional<int> max_value_;  // 新增：整数最大值
    boost::optional<int> min_value_;  // 新增：整数最小值
    boost::optional<int> max_value_;  // 新增：整数最大值
...
public:
    ...
    std::string to_json() const {
        cJSON *json = cJSON_CreateObject();
        std::cout << "PP properties_.size = " << properties_.size() << std::endl;
        for (const auto& property : properties_) {
            // std::cout << "PP to_json:" + property.to_json() << std::endl;
            cJSON *prop_json = cJSON_Parse(property.to_json().c_str());
            // std::cout << "PP name:" + property.name() << std::endl;
            cJSON_AddItemToObject(json, property.name().c_str(), prop_json);
            // std::cout << "add item finish."<< std::endl;
        }
...
public:
    ...
    std::string to_json() const {
        ...
        printf("to_json 003\n");   // properties_.to_json()
        std::cout << "mcptool, to_json:" + properties_.to_json() << std::endl;
        cJSON *properties = cJSON_Parse(properties_.to_json().c_str());
        printf("to_json 003001\n");
    ...
    std::string Call(const PropertyList& properties) {
        std::cout << "Call ... 001" << std::endl;
        ReturnValue return_value = callback_(properties);
        // 返回结果
        std::cout << "Call ... 002" << std::endl;
        cJSON* result = cJSON_CreateObject();
        cJSON* content = cJSON_CreateArray();
        cJSON* text = cJSON_CreateObject();
        cJSON_AddStringToObject(text, "type", "text");
        std::cout << "Call ... 003" << std::endl;
        // if (std::holds_alternative<std::string>(return_value)) {
        if (boost::get<std::string>(&return_value)) {
            std::cout << "Call ... 004" << std::endl;
            cJSON_AddStringToObject(text, "text", boost::get<std::string>(return_value).c_str());
        // } else if (std::holds_alternative<bool>(return_value)) {
        } else if (boost::get<bool>(&return_value)  != nullptr ) {
            std::cout << "Call ... 005" << std::endl;
            cJSON_AddStringToObject(text, "text", boost::get<bool>(return_value) ? "true" : "false");
        // } else if (std::holds_alternative<int>(return_value)) {
        } else if (boost::get<int>(&return_value)  != nullptr) {
            std::cout << "Call ... 006" << std::endl;
            cJSON_AddStringToObject(text, "text", std::to_string(boost::get<int>(return_value)).c_str());
        }
        std::cout << "Call ... 007" << std::endl;
        cJSON_AddItemToArray(content, text);
        cJSON_AddItemToObject(result, "content", content);
        cJSON_AddBoolToObject(result, "isError", false);
        std::cout << "Call ... 008" << std::endl;
        auto json_str = cJSON_PrintUnformatted(result);
        std::string result_str(json_str);
        cJSON_free(json_str);
        cJSON_Delete(result);
        std::cout << "Call ... 009, result_str: " << result_str << std::endl;
        return result_str;
    }
};
...
#define BOARD_NAME "100ask_imx6ull_board"
#define DEFAULT_TOOLCALL_STACK_SIZE 6144

extern std::string mcp_session_id;
...
private:
    McpServer();
    ~McpServer();
    void ParseCapabilities(json &capabilities);
    ...
    void DoToolCall(int id, const std::string &tool_name, json &tool_arguments, int stack_size);
