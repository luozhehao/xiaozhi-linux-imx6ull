...
static bool cache_node_cache_create_cb(lv_freetype_cache_node_t * node, void * user_data)
{
    ...
    FT_Face face;
    LV_LOG_WARN("### FT_New_Face  before...");                      //2025.10.11
    LV_LOG_WARN("### FT_New_Face node->pathname=%s ", node->pathname);   //2025.10.11
    LV_LOG_WARN("### FT_New_Face node->style=%d ", node->style);   //2025.10.11
